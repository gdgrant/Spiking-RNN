...
def calculate_dynamics(prev_eps, x, z, z_prev, z_prev_prev, st, syn_x_prev, h, h_prev, con_dict, eff_var):
    ...
    var_sets = ['inp', 'rec']
    zeds_pre = [x, z_prev]
    zeds_pre_pre = [0, z_prev_prev]
    for v, z_i, z_ii in zip(var_sets, zeds_pre, zeds_pre_pre):
        eps[v] = {}
        ...
        if v != 'inp':
            eps[v]['i'] += h_prev * one_minus_beta * syn_u[v] * syn_x[v] * eff_var[var_name][cp.newaxis,:,:] \
                        * z_ii * one_minus_beta * syn_x_prev
